        ...
        {nodes.map((node, i) => {
          ...
          return (
            <div key={i} className={`relative flex items-center gap-8 md:gap-16 ${isEven ? 'md:flex-row-reverse' : 'md:flex-row'}`}>
              ...
              {/* Node Icon */}
              <motion.div 
                className="relative z-10 w-20 h-20 rounded-2xl glass-dark border border-white/10 flex items-center justify-center bg-black shrink-0"
              ...
              >
                ...
                <node.icon className="w-8 h-8 text-emerald-400" />
              </motion.div>
