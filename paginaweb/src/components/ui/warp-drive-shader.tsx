...
const WarpDriveShader = ({ className = "" }: { className?: string }) => {
  ...
  useEffect(() => {
    ...
    const fragmentShader = `
      ...
      void main() {
        ...
        // Use an emerald/green tint based on Agrilion's aesthetic
        finalColor.r = pow(fract(0.5 / length(uv + vec2(offset, 0.0)) + t * 2.0), 15.0) * 0.1;
        finalColor.g = pow(fract(0.5 / length(uv)                  + t * 2.0), 15.0) * 1.0;
        finalColor.b = pow(fract(0.5 / length(uv - vec2(offset, 0.0)) + t * 2.0), 15.0) * 0.4;
